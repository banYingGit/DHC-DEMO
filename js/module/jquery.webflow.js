+function ($) {
    ...
    var WebFlow = function (element, options) {
        ...
        //弹出窗口
        var sureBtn = $('<a href="javascript:void(0)" class="btn btn-primary" data-role="save">保 存</a>'),

            cancelBtn = $('<a href="javascript:void(0)" class="btn btn-primary-alt" data-role="close">取 消</a>');

        this.$modal = $('<div>').modal({

            title: '设置任务节点'

        }).modal('button', [sureBtn, cancelBtn])
    ...
    WebFlow.prototype = {
        ...
        initGroup: function (width, height) {
            ...
            this.$group.mouseup({inthis: this}, function (e) {
                ...
                if (e.data.inthis.$ghost.css("display") == "none") {
                    ...
                    e.data.inthis.addArea(e.data.in + "_area_" + e.data.inthis.$max, {
        ...
        initWorkForNode: function () {
            ...
            this.$workArea.delegate(".GooFlow_item", "click", {inthis: this}, function (e) {
                ...
                e.data.inthis.$modal.modal('show').modal('load', e.data.inthis.options.infoUrl, {
...
//参数para为JSON结构，与JQUERY中$.ajax()方法的传参一样
        loadDataAjax: function () {
           ...
           // _date: new Data().getTime() 解决ie缓存问题
...
//重构整个流程图设计器的宽高
    ...
    function _buildEvent() {
        ...
        this.$element.on('rename', function (e, obj) {
            ...
            $("[data-id=" + obj.id + "]:first", e.delegateTarget).find('[data-node=name]:first').text(obj.name);
        ...
        this.$modal.on('click', '[data-role=save]', function () {
